    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-vault-bg transition-colors">
      <div className="bg-white dark:bg-vault-surface border border-slate-200 dark:border-vault-border rounded-lg p-8 w-full max-w-md shadow-sm dark:shadow-none">
        ...
        <button
          onClick={handleLogin}
          disabled={loading}
          className="w-full bg-purple-500 dark:bg-vault-gold text-white dark:text-vault-bg font-mono font-bold py-2 rounded hover:bg-purple-600 dark:hover:bg-vault-goldHover transition-colors disabled:opacity-50"
        >
        ...
        <p className="text-slate-500 dark:text-vault-muted text-sm mt-4 text-center">
          No account?{" "}
          <Link to="/register" className="text-purple-500 dark:text-vault-gold hover:underline">Register</Link>
        </p>
      </div>
    </div>
  );
